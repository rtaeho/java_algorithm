...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < M; j++) {
                if (grid[i][j] == '*') {
                    int maxSize = getMaxCrossSize(i, j);
                    if (maxSize > 0) {
                        crosses.add(new Cross(i + 1, j + 1, maxSize));
                        drawCross(i, j, maxSize);
                    }
                }
            }
        }
    ...
    static int getMaxCrossSize(int x, int y) {
        ...
        while (x - size >= 0 && x + size < N && y - size >= 0 && y + size < M &&
               grid[x][y] == '*' && grid[x - size][y] == '*' && grid[x + size][y] == '*' &&
